...
export default function TambahProdukHukum() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const slug = formData.judul.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '') + '-' + Date.now();
    ...
    if (file) {
      ...
      const { error: uploadError } = await supabase.storage
        .from('dokumen') // Kita menggunakan bucket "dokumen"
        .upload(fileName, file);

      if (uploadError) {
        alert('Gagal mengupload dokumen: ' + uploadError.message + '\n\nPastikan bucket "dokumen" sudah dibuat dan diset Public di Supabase!');
        setLoading(false);
        return;
      }
    ...
    const { error } = await supabase.from('produk_hukum').insert([
      { ...formData, slug, file_url: finalFileUrl }
    ]);

    setLoading(false);

    if (error) {
      alert('Gagal menambah produk hukum: ' + error.message);
    } else {
  ...
  return (
    <div className="max-w-3xl animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-3xl font-black text-slate-900">Tambah Produk Hukum</h1>
          <p className="text-slate-500 mt-1">Unggah dokumen baru ke direktori publik.</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input 
              ...
              placeholder="Contoh: Peraturan Lurah No 1 Tahun 2026 tentang Kebersihan"
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    type="text" name="nomor_surat" required placeholder="14"
                ...
                <div>
                  ...
                  <input 
                    type="text" name="tahun" required placeholder="2026"
            ...
            <div className="w-full px-6 py-8 bg-slate-50 border-2 border-dashed border-slate-300 rounded-2xl text-center hover:bg-slate-100 transition-colors relative cursor-pointer">
              ...
              <div className="pointer-events-none flex flex-col items-center justify-center space-y-3">
                ...
                <span className="font-bold text-slate-600">
                  {file ? file.name : "Klik atau Drop file PDF di sini"}
                </span>
                <span className="text-xs font-medium text-slate-400">Maksimal 10MB</span>
              </div>
            </div>
          </div>
